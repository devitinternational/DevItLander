...
function scalePathCoordinates(path: string, scaleX: number): string {
  return path.replace(
...
export default function AnimatedSVGUnderline({
  children,
  ...
  useEffect(() => {
    if (nextIndex === null) setNextIndex(getNextIndex());
  }, [nextIndex]);
  ...
  const pathVariants = useMemo(
    () => ({
      ...
      visible: {
        pathLength: 1,
        pathOffset: 0,
        transition: { type: "tween" as const, duration, ease: "easeInOut" },
      },
      erase: {
        pathLength: 1,
        pathOffset: 1,
        transition: { type: "tween" as const, duration: duration * 0.75, ease: "easeInOut" },
      },
    }),
    [duration]
  );
  ...
  return (
    <motion.button
      className={`relative inline-flex flex-col items-center cursor-pointer bg-transparent border-0 ${className}`}
